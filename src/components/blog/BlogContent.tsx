...
import { BLOG_POSTS, BLOG_CATEGORIES, FEATURED_POST, type BlogCategory } from '@/lib/blog-data'
        ...
        <div className="page-content">
          ...
          <section style={{ maxWidth: 1240, margin: '0 auto', padding: '46px 24px 0' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 400px), 1fr))', gap: 22, alignItems: 'stretch' }}>
              <Link href={routes.blogPost(FEATURED_POST.slug)} style={{ display: 'flex', flexDirection: 'column', borderRadius: 24, overflow: 'hidden', background: '#FFFFFF', border: '1px solid rgba(4,18,31,0.09)', boxShadow: '0 16px 40px rgba(4,18,31,0.07)' }}>
                <div style={{ height: 280, background: 'linear-gradient(150deg, #1B7A3D, #08361B)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: 26 }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-lato), Lato, sans-serif', fontWeight: 900, fontSize: 27, lineHeight: 1.14, letterSpacing: '-0.7px', color: '#FFFFFF' }}>{FEATURED_POST.title}</span>
                </div>
                <div style={{ padding: '24px 26px 26px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                  <p style={{ margin: 0, fontSize: 15, lineHeight: 1.6, color: '#4B5D6E' }}>{FEATURED_POST.excerpt}</p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 4 }}>
                    <span style={{ width: 34, height: 34, borderRadius: '50%', background: 'var(--moss-light)', display: 'grid', placeItems: 'center', fontFamily: 'var(--font-lato), Lato, sans-serif', fontWeight: 700, fontSize: 12.5, color: '#16210B' }}>{FEATURED_POST.initials}</span>
                    <span style={{ fontSize: 13, color: '#6A7F92' }}>{FEATURED_POST.author} · {FEATURED_POST.date} · {FEATURED_POST.readTime}</span>
                  </div>
                </div>
              </Link>
